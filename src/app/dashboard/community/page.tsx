...
import CommunityCard, { CommunityCardType } from "@/app/components/community-card/community-card";
import Icon from "@/app/components/icon/icons";
import { Input } from "@/components/ui/input";
import CommunityListView from "@/app/components/community-listview/community-listview";

const Community = () => {
	const dispatch = useDispatch();
	const [visibleCount, setVisibleCount] = useState(4);
	const [communitiesData, setCommunitiesData] = useState([]);
	const [filteredData, setFilteredData] = useState<CommunityCardType[]>([]);
	...
	const getCommunityData = async () => {
		try {
			...
			const response = await axios.get(`${DOMAIN}/api/common/get-communities`);
	...
	useEffect(() => {
		if (searchQuery) {
			const filtered = communitiesData.filter((pilot: CommunityCardType) =>
				pilot.communityName.toLowerCase().includes(searchQuery.toLowerCase())
			);
	...
	return (
		<div className="">
			...
			{view === "grid" ? (
				<>
					<div className="p-30">
						<div className="flex flex-wrap justify-between gap-30">
							{filteredData.slice(0, visibleCount).map((comm: CommunityCardType, index: number) => {
								return (
									<CommunityCard
										key={index}
										communityName={comm.communityName}
										postedDate={comm.postedDate}
										members={comm.members}
										communityImage={comm.communityImage}
									/>
								);
							})}
						</div>
				...
				<div className="p-30">
					<CommunityListView communities={filteredData} />
				</div>
			)}
		</div>
	);
